    ...
    with patch('junction_analisys._analyze_single_cluster', side_effect=mock_analyze):
        with patch('junction_analisys.build_exon_lookup') as mock_exon_lookup:
            with patch('junction_analisys.build_domain_lookup') as mock_domain_lookup:
                ...
                # Call the worker
                results = _process_cluster_chunk(
                    chunk_info,
                    df_exons=df_exons,
...
# ---------------------------------------------------------------------------
# use_longest_cds tie-break determinism
# ---------------------------------------------------------------------------

def test_analyze_junction_longest_cds_tiebreak_is_deterministic():
    """
    When two transcripts both qualify for comparison and have EQUAL CDS
    length, the choice between them must be deterministic (not dependent on
    dict/set iteration order or the per-process string hash seed).
    """
    ...
    cluster_result.analyze_junction(
        ...
        exon_lookup=exon_lookup, domain_lookup=domain_lookup, use_longest_cds=True,
    )

    df_results = cluster_result.get_results_df()
    skipped = df_results[df_results['event'] == 'skipped_not_longest_cds']['transcript_id'].tolist()
    compared = df_results[df_results['event'] != 'skipped_not_longest_cds']['transcript_id'].dropna().tolist()

    # max() with key=(cds_length, tid) picks the lexicographically larger id on a tie.
    assert skipped == ['ENST_AAA']
    assert compared == ['ENST_BBB']
